...
app.get("/fileman", cors(CORSOPTIONS), async (req, res) => {
  ...
  res.json({
    dirname: path.basename(qpath),
    prevDir: path.resolve(qpath, ".."),
    cwd: qpath,
    directories,
    files,
  });
...
async function getCurrentTracks() {
  return {
    video: await handle(mpv.getProperty(`current-tracks/video`)).then(
      (resp) => resp[0]
    ),
    audio: await handle(mpv.getProperty(`current-tracks/audio`)).then(
      (resp) => resp[0]
    ),
    subtitle: await handle(mpv.getProperty(`current-tracks/sub`)).then(
      (resp) => resp[0]
    ),
  };
}
  ...
  for (let i = 0; i < count; i++) {
    try {
      tracks.push({
        ...
        lang: await handle(mpv.getProperty(`track-list/${i}/lang`)).then(
          (resp) => resp[0]
        ),
        external_filename: await handle(
        ...
        demuxW: await handle(mpv.getProperty(`track-list/${i}/demux-w`)).then(
          (resp) => resp[0]
        ),
        demuxH: await handle(mpv.getProperty(`track-list/${i}/demux-h`)).then(
          (resp) => resp[0]
        ),
        demuxChannelCount: await handle(
          mpv.getProperty(`track-list/${i}/demux-channel-count`)
        ).then((resp) => resp[0]),
        demuxChannels: await handle(
          mpv.getProperty(`track-list/${i}/demux-channels`)
        ).then((resp) => resp[0]),
        demuxSamplerate: await handle(
          mpv.getProperty(`track-list/${i}/demux-samplerate`)
        ).then((resp) => resp[0]),
        demuxBitrate: await handle(
  ...
  for (let i = 0; i < count; i++) {
    try {
      playlist.push({
        ...
        filename: await handle(mpv.getProperty(`playlist/${i}/filename`)).then(
          (resp) => resp[0]
        ),
